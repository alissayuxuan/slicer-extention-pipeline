...
#edited version just to run model creation...
def run_pipeline_a_patient():
    work_directory = entries['work_directory'].get()
    patient_data_directory = entries['patient_data_directory'].get()
    patient_data_output_directory = entries['patient_data_directory'].get()
    model_creation_no = selected_var.get()
    try:
        patient_ID = os.path.split(patient_data_directory)[1]  #assumes patientID is the folder name
        print(patient_ID)
        patient_directory_path = patient_data_directory
        model_creation_setup_path = os.path.join(work_directory,r'base/setup/model_creation_base_setup.xml')  #this can't be work directory!
        ...
        run_pipeline(work_directory,model_creation_setup_path,patient_setup_path,model_creation_no)
        tk.messagebox.showinfo('Success','Done.')
    except Exception as e:
        tk.messagebox.showinfo('Error',f'An error happened:\nPatient ID: {patient_ID}\n{e}.')

def run_pipeline(work_directory,model_creation_setup_path,patient_setup_path,model_creation_no = '0'):
    if model_creation_no=='0':
        model_creation_script_path = os.path.join(work_directory,'opensim_pipeline_3d/Header_CreateModel_3D.py')
...
def print_setup_a_patient():
    work_directory = entries['work_directory'].get()
    patient_data_directory = entries['patient_data_directory'].get()
    patient_data_output_directory = entries['patient_data_directory'].get()
    
    patient_ID_display = ''
    patient_ID = os.path.split(patient_data_directory)[1]
    patient_directory_input_path = patient_data_directory
    patient_directory_output_path =patient_data_output_directory
...
def print_setup(patient_directory_input_path,patient_directory_output_path,patient_ID):
    ...
    model_creation_patient_node.append(etree.Comment('Info file path (.json)'))
    ...
    try:
        create_folder([output_model_path,output_setup_path])
    except Exception as e:
        tk.messagebox.showinfo('Info','Output folders already exist.')
...
entries = {}
global_variables = [
    ('Work Directory (opensim_pipeline folder)'                           ,'work_directory'           , 'Directory'   ,''),
    ('Patient Data Directory (for one patient)'                   ,'patient_data_directory'   , 'Directory'   ,''),
]
    #('Patient Data Output Directory'            ,'patient_data_output_directory','Directory',r'result')
variables = [
    ('Info File Path'                           ,'info_file_path'           , 'Entry'       ,r'osim_info.json'),
    ('Calibration Trial Path'                   ,'calibration_trial_path'   , 'Entry'       ,r''),
    ('Model Output Path'                        ,'output_model_path'        , 'Entry'       ,r''),
    ('Fullyscaled Model Output Path'            ,'output_scaled_model_path' , 'Entry'       ,r''),
    ('Setup Output Path'                        ,'output_setups_path'       , 'Entry'       ,r'setup'),
]
   #('Spine Loading Output Path'                ,'output_spine_loading_path', 'Entry'       ,r'results')

root = tk.Tk()
root.title('Model Creation Setup & Run for a Patient')
...
btn_submit = tk.Button(root, text='Save Setup File', command=print_setup_a_patient)
btn_submit.pack(pady=20)

label4 = tk.Label(root, text='Run Model Creation',font='Arial 14 bold')
label4.pack(pady=15)
...
radio1 = tk.Radiobutton(frame, text='3D CT (Default)', variable=selected_var, value=0)
radio2 = tk.Radiobutton(frame, text='Marker (future dev)', variable=selected_var, value=1)
# may want to add a Marker + no CT version here....
radio1.grid(row=0,column=0, padx=10)
radio2.grid(row=0,column=1, padx=10)

btn_run_pipeline = tk.Button(root, text='RUN', command=run_pipeline_a_patient)
